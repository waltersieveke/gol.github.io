...
if (range) {
    range.addEventListener("input", (RANGE) => rangeSlider(RANGE));
}
//die Zellbreite wird mit dem regler geändert
function rangeSlider(inputellll) {
    ...
    if (ctx) {
        for (loopx = 0; loopx < anzahlFelderLaenge; loopx++) {
            for (loopy = 0; loopy < anzahlFelderBreite; loopy++) {
                ctx.strokeRect(0 + loopx * zellBreite, 0 + loopy * zellLaenge, zellBreite, zellLaenge);
            }
        }
    }
}
...
function initFieldasFalse(field) {
...
if (btn) {
    btn.addEventListener("click", (nextGen) => Generationswechsel());
}
let count = 1;
let ende = false;
let generationen = 0;
...
function Generationswechsel() {
    ...
    if (ende == true) {
        ...
        ctx === null || ctx === void 0 ? void 0 : ctx.clearRect(0, 0, feldlänge, feldbreite); //die voherige Generation wird gelöscht damit die nächtse angezeigt werden kann
        rules();
        if (draw == 0) {
...
if (btn1) {
    btn1.addEventListener("click", (startStopp) => buttonTrueFalse());
}
...
if (speed) {
    speed.addEventListener("input", (RANGEspeed) => changespeed(RANGEspeed));
}
//altes interval wird gecleart und neues durch das value des reglers angepasst
let intervalId2 = 0;
let timeInterval;
function changespeed(inputellllll) {
...
if (colorpicker) {
    colorpicker.addEventListener("input", (COLORlist) => changecolor(COLORlist));
}
//farben auf die Zellen anwenden, auf direkte änderung der Farbe auch direkte änderung der Zelle
function changecolor(clickell) {
    //console.log("changeColor");
    if (ctx) {
        color = clickell.explicitOriginalTarget.value;
        ctx === null || ctx === void 0 ? void 0 : ctx.clearRect(0, 0, feldlänge, feldbreite);
        drawBoard(feld);
        createCanvas();
    }
}
...
if (random) {
    random.addEventListener("click", (Random) => randomLife());
}
...
if (restbutton) {
    restbutton.addEventListener("click", (RESET) => clear());
}
...
document.addEventListener("click", (MOUSEclick) => showCoords(MOUSEclick));
//beim clicken werden die koordinaten angezeigt
function showCoords(event) {
    //console.log("showCoords");
    let x = event.clientX;
    let y = event.clientY + scrollY;
    let Yfeld = ((y - event.explicitOriginalTarget.offsetTop) /
        zellBreite).toString();
...
if (saveButton) {
    saveButton.addEventListener("click", (SAVE) => saveGame());
}
function saveGame() {
...
if (loadButton) {
    loadButton.addEventListener("click", (load) => loadGame());
}
function loadGame() {
...
function displayGenerationenScore() {
    ...
    if (generation) {
        ...
        generation.innerHTML = generation.innerHTML.replace(temp, "Generation: " + generationen);
    }
}
//button für die randomRainbow felder
let rainbowBTN = document.getElementById("Rainbow");
if (rainbowBTN) {
    rainbowBTN.addEventListener("click", (RAINBOW) => drawRandomColor(feld));
}
...
function drawRandomColor(fieldvar) {
...
if (darkBtn) {
    darkBtn.addEventListener("click", (DARK) => darkmode());
}
function darkmode2() {
    if (p) {
